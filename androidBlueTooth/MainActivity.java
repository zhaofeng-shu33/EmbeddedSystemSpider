    ...
    private Runnable sendDataRunnable = new Runnable()
    {
        ...
        public void run()
        {
            try {
                ...
                byte[] toSend = new byte[8];//一个byte 是10进制的0~255
                toSend[0] = (byte)255;//第一位用于校验
                toSend[1] = (byte)(last_st_x);//左边手柄的(x,y)坐标，是左手坐标系
                toSend[2] = (byte)(last_st_y);//y轴正方向向下，tosend[1]到4初始值为16进制64
                toSend[3] = (byte)(last_th_x);//右边手柄的(x,y)坐标，0x64=6*16+4=100//sensor 1开关
                toSend[4] = (byte)(last_th_y);//th 是 throttle 缩写
                toSend[5] = (byte)(sensor1);//sensor 1开关，不开启为00,开启为01
                toSend[6] = (byte)(sensor2);//sensor 2开关,初始值为0
        ...
        timerSendDataHandler.postDelayed(this, 20);//每隔20ms发送一次数组byte.
        }
    };
